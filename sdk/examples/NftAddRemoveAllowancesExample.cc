...
int main(int argc, char** argv)
{
  ...
  const std::string cid = "QmNPCiNA3Dsu3K5FxDPMG5Q3fZRwVTg14EXA92uqEeSRXn";
  ...
  // Define a uniform distribution to generate integers in a range
  std::uniform_int_distribution<int> distribution(1, 5);

  //Populate CIds
  int entries = 1000;
  for(auto i = 0; i < entries; i++){
    std::string buildCid = cid + std::to_string(i);
    // Generate a random number to pad the CIDs for length randomness
    int randomPadSize = distribution(gen);
    buildCid += buildCid + std::string(randomPadSize, '0');
    CIDs.push_back(buildCid);
  }
  ...
  for (const auto& metadata : CIDs)
  {
    ...
    std::cout<<std::endl<<"Token Mint transaction(executor, tokenid, ms): "<< std::endl;
    std::cout<< testTxResponse.mNodeId.toString() << " " << tokenId.toString() <<" "<< std::to_string(serialNumbers.back()) <<" "<< std::to_string(elapsed.count()) << std::endl<<std::endl;
