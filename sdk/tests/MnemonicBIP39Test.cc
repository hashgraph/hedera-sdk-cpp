...
{
  auto paramVector = GetParam();

  std::string inputEntropy = get<0>(paramVector);
  std::string mnemonicString = get<1>(paramVector);
  std::string seed = get<2>(paramVector);

  MnemonicBIP39 mnemonicFromEntropy = MnemonicBIP39::initializeBIP39Mnemonic(
    MnemonicBIP39::entropyToWordIndices(HexConverter::hexToBase64(inputEntropy)));
  ...
  ASSERT_EQ(mnemonicFromEntropy.toSeed(PASSPHRASE), HexConverter::hexToBase64(seed));
}
